...
import { Property, HeroSearchState } from './types'; // Asegúrate de que HeroSearchState esté exportado en types.ts

const UF_VALUE_CLP = 37800;

// Definición de un estado inicial seguro para los filtros
...
const App: React.FC = () => {
  ...
  // 👈 Inicialización corregida a INITIAL_SEARCH_STATE
  const [searchFilters, setSearchFilters] = useState<HeroSearchState>(INITIAL_SEARCH_STATE);
  ...
  const handleNavigate = (view: string, category: string = '') => {
    setCurrentView(view);
    setSelectedCategory(category);
    // 👈 Resetea los filtros solo si no vas a una vista de búsqueda o listado
    if (view === 'home' || view === 'showroom' || view === 'admin') {
  ...
  // 👈 Función para limpiar filtros del ListingView, resetea el estado
  const handleClearFilters = () => {
    setSearchFilters(INITIAL_SEARCH_STATE);
    // Vuelve a la categoría principal, forzando la re-ejecución del filtro.
    setSelectedCategory('Bienes Raíces'); 
  };


  const getFilteredProperties = () => {
    let filtered = properties.filter(p => p.status === 'published'); // Cambiado para usar status
    ...
    if (selectedCategory && selectedCategory !== 'Resultados de Búsqueda') {
      if (selectedCategory === 'Bienes Raíces' || selectedCategory === 'Desarrollos') {
        // No se aplica filtro adicional por categoría principal aquí
      } else if (selectedCategory === 'Premium Property') {
          filtered = filtered.filter(p => p.isPremium);
      } else {
        // Asume que la categoría es una comuna/localización
        filtered = filtered.filter(p => p.location.includes(selectedCategory));
      }
    }
    
    // Aplicar filtros de HeroSearchState (que ya están en el estado 'searchFilters')
    // Nota: ListingView también hace un filtrado local, pero este filtrado inicial asegura que la lista enviada sea relevante
    if (searchFilters) {
          ...
          <>
             ...
             <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
                 {properties.filter(p => p.status === 'published').slice(0, 6).map(property => (
      ...
      case 'listing':
        return (
          <ListingView 
            ...
            // 👈 Propiedades añadidas para que ListingView compile
